 ...
 * Returns JSON:      { success, cart_count, cart_total, message }
 *
 * Supports both:
 *   - Logged-in users  → stored in `cart` DB table
 *   - Guests           → stored in $_SESSION['guest_cart']
 */
...
if ($is_guest) {
    ...
    $current_qty = (int) ($_SESSION['guest_cart'][$product_id] ?? 0);
    $new_qty     = min($current_qty + $quantity, $stock);

    if ($new_qty === $current_qty && $current_qty > 0) {
        json_out(['success' => false, 'message' => 'You already have the maximum available quantity in your cart']);
    }

    $_SESSION['guest_cart'][$product_id] = $new_qty;

    // Calculate guest cart totals from session
    $cart_count = 0;
    $cart_total = 0.0;
    ...
    if (!empty($_SESSION['guest_cart'])) {
        ...
        $stmt = $pdo->prepare("SELECT id, price FROM products WHERE id IN ($ph) AND stock_count > 0");
        $stmt->execute($ids);
        $prices = $stmt->fetchAll(PDO::FETCH_KEY_PAIR); // id => price

        foreach ($_SESSION['guest_cart'] as $pid => $qty) {
            if (isset($prices[$pid])) {
                $cart_count += $qty;
                $cart_total += $qty * (float) $prices[$pid];
            }
        }
    }
...
try {
    ...
    json_out(['success' => false, 'message' => 'Could not update cart'], 500);
}
