...
$user_id      = $is_guest ? null : (int)$_SESSION['user_id'];
...
// ── Fetch cart items ─────────────────────────────────────────────
// Guests: cart comes from session + we re-validate prices from DB
// Logged-in: fetch fresh from DB cart table
if ($is_guest) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT id AS product_id, name, brand,
        ...
        foreach ($products as $p) {
            $qty = (int)($guest_cart[$p['product_id']] ?? 1);
            $cart_items[] = [
                ...
                'item_total'    => (float)$p['current_price'] * $qty,
            ];
        }
    } catch (PDOException $e) {
        error_log('Guest cart fetch error: ' . $e->getMessage());
        echo json_encode(['success' => false, 'message' => 'Could not read cart.']);
        exit();
    }
} else {
    try {
...
// ── Migrate existing table (safe, runs every request) ───────────
try { $pdo->exec("ALTER TABLE `orders` ADD COLUMN `is_guest` TINYINT(1) NOT NULL DEFAULT 0"); } catch (PDOException $e) {}
try { $pdo->exec("ALTER TABLE `orders` MODIFY COLUMN `user_id` INT UNSIGNED NULL"); } catch (PDOException $e) {}
...
try {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO `orders`
            (user_id, order_number, full_name, phone, email, subtotal, shipping_cost, total, currency, status, is_guest, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, NOW())
    ");
    $stmt->execute([$user_id, $order_number, $full_name, $phone, $email,
                    $db_subtotal, $shipping, $db_total, $currency, $is_guest ? 1 : 0]);
    ...
    // Clear cart — DB for logged-in, session for guests
    if ($is_guest) {
        $_SESSION['guest_cart'] = [];
    } else {
...
$guest_badge = $is_guest ? '<span style="background:#fef3c7;color:#d97706;font-size:11px;font-weight:700;padding:2px 8px;border-radius:100px;margin-left:8px;">GUEST</span>' : '';
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:16px;overflow:hidden;">
  <tr><td style="background:#0cb100;padding:24px 32px;">
    <div style="color:#fff;font-size:20px;font-weight:800;">🛒 New Order Received <?=$guest_badge?></div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;margin-bottom:16px;">
      <tr><td style="padding:16px 20px;">
        ...
        <div style="font-size:14px;color:#0f172a;line-height:1.9;">
          ...
          <strong>Type:</strong> <?=$is_guest ? '👤 Guest Order' : '✅ Registered User'?>
        </div>
      </td></tr>
    </table>
...
$subject_admin    = "New Order #{$order_number} from {$safe_name}" . ($is_guest ? ' [Guest]' : '');
...
/*
// PHPMailer Gmail SMTP — uncomment to use instead of mail()
