...
// ── Guest session init ──────────────────────────────────────────
// Allow guest cart stored in session (no login required)
...
// ── Fetch cart items ────────────────────────────────────────────
if ($is_guest) {
    // Build cart from session + product lookup
...
// ── Totals (calculated AFTER cart items are fetched) ────────────
...
<section class="page-header">
    <div class="container">
        ...
        <div class="header-content">
            ...
            <?php if ($is_guest): ?>
            <!-- Guest banner in header
            <div class="guest-header-badge">
                <i class="fas fa-user-clock"></i>
                <span>Guest Session</span>
                <a href="login.php?redirect=cart.php" class="guest-signin-link">Sign in to save</a>
            </div>-->
            <?php endif; ?>
        </div>
    </div>
...
<!-- ── Guest Notice Bar (shown only to guests with items) ──────── -->
<?php if ($is_guest && !empty($cart_items)): ?>
<!--<div class="guest-notice-bar">
    <div class="container">
        <div class="gnb-inner">
            <div class="gnb-left">
                <div class="gnb-icon"><i class="fas fa-info-circle"></i></div>
                <div>
                    <strong>Shopping as a guest</strong>
                    <span>Your cart is saved for this session only.</span>
                </div>
            </div>
            <div class="gnb-actions">
                <a href="login.php?redirect=cart.php" class="gnb-btn gnb-btn-primary">
                    <i class="fas fa-sign-in-alt me-1"></i>Sign In
                </a>
                <a href="register.php?redirect=cart.php" class="gnb-btn gnb-btn-outline">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                <div class="summary-card">
                    ...
                    <?php if ($is_guest): ?>
                    <!-- Guest login nudge inside summary 
                    <div class="summary-login-nudge">
                        <div class="sln-icon"><i class="fas fa-shield-alt"></i></div>
                        <div>
                            <div class="sln-title">Save your cart</div>
                            <div class="sln-desc">Sign in to keep your cart across sessions and track orders.</div>
                        </div>
                        <a href="login.php?redirect=cart.php" class="sln-btn">Sign In</a>
                    </div>-->
                    <?php endif; ?>
...
<div id="orderModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
    <div class="modal-box" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
        ...
        <div class="modal-body">
            <!-- ── Guest login prompt (shown to guests only) ── -->
            <?php if ($is_guest): ?>
            <div class="modal-guest-bar">
                <div class="mgb-inner">
                    <div class="mgb-text">
                        <i class="fas fa-user-circle"></i>
                        <span>Have an account? <a href="login.php?redirect=cart.php">Sign in</a> for faster checkout and order tracking.</span>
                    </div>
            ...
            <div id="modalSuccessStep" style="display:none;" class="modal-success">
                ...
                <div class="success-guest-cta">
                    <p class="sgc-text">Create a free account to track this order and save your details for next time.</p>
                    <a href="register.php" class="sgc-btn">
                        <i class="fas fa-user-plus me-1"></i>Create Account
                    </a>
                </div>
                <?php endif; ?>
                <div class="redirect-countdown">
                    ...
                    <span id="countdownText">Redirecting in <strong id="countdownNum">3</strong>s…</span>
                </div>
            </div>
        </div>
    </div>
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .guest-header-badge i { color: var(--guest); }
    .guest-signin-link {
        color: #fff;
        font-weight: 600;
    ...
    .guest-signin-link:hover { color: #fde68a; }
    ...
    .gnb-left { display: flex; align-items: center; gap: .85rem; }
    .gnb-icon {
        width: 38px; height: 38px;
        background: rgba(245,158,11,0.15);
        border-radius: 50%;
        display: flex; align-items: center; justify-content: center;
        color: var(--guest); font-size: 1rem; flex-shrink: 0;
    }
    .gnb-left strong { display: block; font-size: .9rem; color: var(--text-primary); }
    .gnb-left span { font-size: .82rem; color: var(--text-secondary); }
    .gnb-actions { display: flex; gap: .6rem; flex-shrink: 0; }
    .gnb-btn {
    ...
    .gnb-btn-primary { background: var(--guest); color: white; border: none; }
    .gnb-btn-primary:hover { background: #d97706; color: white; }
    .gnb-btn-outline { background: none; color: var(--guest); border: 1.5px solid var(--guest); }
    .gnb-btn-outline:hover { background: var(--guest); color: white; }
    ...
    .guest-label {
        ...
        background: var(--guest-light); color: var(--guest);
    ...
    .btn-checkout {
        ...
        cursor: pointer; transition: all .25s; margin-bottom: .75rem; letter-spacing: -.01em;
    }
    ...
    /* Summary login nudge for guests */
    .summary-login-nudge {
    ...
    .sln-icon { color: var(--guest); font-size: 1.25rem; flex-shrink: 0; }
    .sln-title { font-size: .83rem; font-weight: 700; color: var(--text-primary); margin-bottom: .1rem; }
    .sln-desc { font-size: .75rem; color: var(--text-secondary); line-height: 1.4; }
    .sln-btn {
        margin-left: auto; flex-shrink: 0;
        background: var(--guest); color: white;
        border: none; border-radius: 8px;
        padding: .4rem .85rem; font-size: .78rem; font-weight: 600;
        text-decoration: none; white-space: nowrap; transition: background .2s;
    }
    .sln-btn:hover { background: #d97706; color: white; }
    ...
    .modal-box { background: var(--surface); border-radius: var(--radius-xl); width: 100%; max-width: 520px; box-shadow: 0 24px 64px rgba(0,0,0,0.22); overflow: hidden; animation: modalSlideUp .35s cubic-bezier(0.34,1.56,0.64,1); }
    @keyframes modalSlideUp { from { transform: translateY(40px) scale(.97); opacity: 0; } to { transform: translateY(0) scale(1); opacity: 1; } }
    .modal-header { display: flex; align-items: center; gap: 1rem; padding: 1.5rem 1.5rem 1rem; border-bottom: 1px solid var(--border); }
    ...
    .mgb-dismiss:hover { color: var(--text-primary); }
    ...
    #countdownText { font-size: .82rem; color: var(--text-muted); }
    ...
    @media (max-width: 768px) {
        ...
        .guest-header-badge { display: none; }
        .gnb-inner { flex-direction: column; align-items: flex-start; }
    }
</style>
...
function addToCart(productId, quantity = 1) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Guest-friendly: don't force login just for recommendations
...
function openOrderModal() {
    ...
    `).join('');
    const modalTotal = CART_ITEMS.reduce((sum, item) => sum + item.item_total, 0) + SHIPPING_COST;
    document.getElementById('mosTotal').textContent = CURRENCY + ' ' + numFmt(modalTotal);

    ['orderFullName','orderPhone','orderEmail'].forEach(id => {
        document.getElementById(id).value = '';
        document.getElementById(id).classList.remove('is-invalid');
    });
    ['nameError','phoneError','emailError'].forEach(id => { document.getElementById(id).textContent = ''; });
...
function validateOrderForm() {
    ...
    if (!fullName) { setFieldError('orderFullName','nameError','Full name is required.'); valid = false; }
    else clearFieldError('orderFullName','nameError');
    if (!phone) { setFieldError('orderPhone','phoneError','Phone number is required.'); valid = false; }
    else if (!/^[\d\s\+\-\(\)]{7,20}$/.test(phone)) { setFieldError('orderPhone','phoneError','Please enter a valid phone number.'); valid = false; }
    else clearFieldError('orderPhone','phoneError');
    if (!email) { setFieldError('orderEmail','emailError','Email address is required.'); valid = false; }
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setFieldError('orderEmail','emailError','Please enter a valid email address.'); valid = false; }
    else clearFieldError('orderEmail','emailError');
    return valid;
}
function setFieldError(inputId, errorId, msg) { document.getElementById(inputId).classList.add('is-invalid'); document.getElementById(errorId).textContent = msg; }
function clearFieldError(inputId, errorId) { document.getElementById(inputId).classList.remove('is-invalid'); document.getElementById(errorId).textContent = ''; }
...
function confirmOrder() {
    ...
    const payload = {
        ...
        subtotal: CART_SUBTOTAL, shipping: SHIPPING_COST, total: CART_TOTAL,
        currency: CURRENCY,
        is_guest: IS_GUEST
    };
    ...
    .then(data => {
        if (data.success) {
            ...
            // Clear guest cart from session after success via small AJAX call
            if (IS_GUEST) {
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.cart-card').forEach((card, i) => { card.style.animationDelay = `${i * 80}ms`; });
