...
// ── Extract warranty from specs ───────────────────────────────────────────────
$warrantyRaw  = '';
foreach ($specs as $key => $val) {
    if (stripos($key, 'warranty') !== false) {
        $warrantyRaw = $val;
        break;
    }
}

// Normalise to "X Year(s)" regardless of what the spec value contains
// Handles: "2", "2 Years", "2 Year", "24 months", "Two years", etc.
$warrantyYears = 1; // default
if ($warrantyRaw) {
    // Extract leading number or convert month count
    if (preg_match('/(\d+)\s*month/i', $warrantyRaw, $m)) {
        $warrantyYears = max(1, (int)round((int)$m[1] / 12));
    } elseif (preg_match('/(\d+)/', $warrantyRaw, $m)) {
        $warrantyYears = max(1, (int)$m[1]);
    }
}
$warrantyText = $warrantyYears . ' ' . ($warrantyYears === 1 ? 'Year' : 'Years');
...
<style>
    ...
    .det-stock-count {
        ...
        padding:.38rem .9rem; border-radius:var(--r-full);
        font-size:.82rem; font-weight:700;
        background:rgba(79,70,229,.08); color:var(--accent);
    ...
    .det-warranty-badge i { font-size:.78rem; }
    ...
    .btn-det-quote {
        ...
        border:2px solid var(--accent); border-radius:var(--r-md); cursor:pointer;
        font-family:'Red Hat Display',sans-serif; font-size:.92rem; font-weight:700;
    ...
    .quote-modal {
        ...
        box-shadow:0 24px 80px rgba(10,10,15,.22);
    ...
    .qsub { font-size:.8rem; color:var(--ink-3); margin:0; }
    .quote-close {
    ...
    .quote-close:hover { background:rgba(220,38,38,.1); color:#dc2626; }
    ...
    .qm-prod-img img { max-width:100%; max-height:100%; object-fit:contain; }
    .qm-prod-fallback { width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:var(--ink-3); font-size:1.4rem; }
    .qm-prod-brand { font-size:.68rem; font-weight:700; color:var(--ink-3); text-transform:uppercase; letter-spacing:.06em; margin-bottom:.2rem; }
    .qm-prod-name { font-size:.88rem; font-weight:700; color:var(--ink); line-height:1.35; margin-bottom:.25rem; }
    .qm-prod-price { font-size:.9rem; font-weight:800; color:var(--accent); }

    /* Form fields */
    .quote-modal #qFormStep,
    .quote-modal .qm-success { padding:0 1.5rem 1.5rem; }

    .q-field { margin-bottom:.9rem; }
    .q-field label {
        display:flex; align-items:center; gap:.35rem;
        font-size:.78rem; font-weight:700; color:var(--ink-2); margin-bottom:.35rem;
    }
    .q-field label i { color:var(--accent); font-size:.72rem; }
    .qreq { color:#dc2626; }
    .q-field input, .q-field textarea {
        width:100%; padding:.62rem .85rem;
        border:1.5px solid rgba(0,0,0,.1); border-radius:var(--r-sm);
        font-family:'Red Hat Display',sans-serif; font-size:.875rem; color:var(--ink);
        background:var(--surface);
    ...
    .q-field input.q-invalid, .q-field textarea.q-invalid {
        border-color:#dc2626;
        box-shadow:0 0 0 3px rgba(220,38,38,.08);
    }
    .q-field textarea { resize:vertical; min-height:76px; }
    .q-err { font-size:.75rem; color:#dc2626; margin-top:.28rem; min-height:.9rem; }
    .q-row2 { display:grid; grid-template-columns:1fr 1fr; gap:.75rem; }
    ...
    .q-validity-note i { flex-shrink:0; }

    /* Buttons */
    .q-actions { display:flex; gap:.65rem; }
    .btn-q-cancel {
        flex:1; padding:.7rem;
        border:1.5px solid rgba(0,0,0,.1); border-radius:var(--r-md);
        background:none; font-family:'Red Hat Display',sans-serif;
        font-size:.9rem; font-weight:600; color:var(--ink-2); cursor:pointer;
        transition:background .15s, color .15s;
    }
    .btn-q-cancel:hover { background:var(--surface); color:var(--ink); }
    .btn-q-submit {
        flex:2; padding:.7rem;
    ...
    .btn-q-submit:hover:not(:disabled) { background:var(--accent-dark); transform:translateY(-1px); }
    .btn-q-submit:disabled { background:#ccc; box-shadow:none; cursor:not-allowed; }

    /* Success step */
    .qm-success { text-align:center; padding-top:1rem !important; }
    .qm-success-icon { font-size:3.2rem; color:var(--accent); margin-bottom:.9rem; animation:successPop .5s cubic-bezier(.34,1.56,.64,1); }
    @keyframes successPop { from{transform:scale(.5);opacity:0} to{transform:scale(1);opacity:1} }
    .qm-success h4 { font-family:'Red Hat Display',sans-serif; font-size:1.2rem; font-weight:900; margin:0 0 .5rem; color:var(--ink); }
    .qm-success p { font-size:.875rem; color:var(--ink-2); line-height:1.65; margin-bottom:1.25rem; }

    @media (max-width:520px) {
        .q-row2 { grid-template-columns:1fr; }
        .qm-product-strip { flex-direction:column; align-items:flex-start; }
    }
    ...
    /* ── Specs table — clean key-value style ── */
    .specs-wrap {
        background:#fff;
    ...
    .specs-section-title {
        ...
        letter-spacing:-.02em; padding:1.4rem 1.6rem 1rem;
        border-bottom:2px solid #e4e4ec;
    }
    .specs-table { width:100%; border-collapse:collapse; }
    .specs-table tr { border-bottom:1px solid #e4e4ec; transition:background .12s; }
    .specs-table tr:last-child { border-bottom:none; }
    .specs-table tr:hover td { background:#f6f6fb; }
    .specs-table td {
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-7 col-md-6">
                    <div class="info-panel">
                        ...
                        <!-- Stock status + Stock count + Warranty badge row -->
                        <div class="det-status-row">
                            ...
                            <?php if ($product['in_stock'] && $product['stock_count'] > 5): ?>
                            <?php endif; ?>

                            <span class="det-warranty-badge">
                                <i class="fas fa-shield-halved"></i>
                                <?= htmlspecialchars($warrantyText) ?> Warranty
                            </span>
                        </div>
                        ...
                        <div class="det-actions">
                            ...
                            <button class="btn-det-quote" onclick="openQuoteModal()">
                                <i class="fas fa-file-invoice"></i> Get a Quote
                            </button>
                        </div>

                        <!-- Meta chips (category only) -->
                        <div class="det-meta">
    ...
    <div class="det-tabs">
        <div class="container">
            ...
            <div class="tab-panel active" id="tab-specs">
                <?php if (!empty($specs) || $product['brand'] || $product['category']): ?>
                <div class="specs-wrap">
                    <div class="specs-section-title">Specifications</div>
                    <table class="specs-table">
                        ...
                        <?php if ($product['category']): ?>
                            <tr>
                                <td>Category</td>
                                <td><?php echo htmlspecialchars(ucwords(str_replace(['_','-'],' ',$product['category']))); ?></td>
                            </tr>
                        <?php endif; ?>
                        <?php foreach ($specs as $key => $val):
                            // Skip brand/category if already shown above
                            if (strtolower($key) === 'brand'    && $product['brand'])    continue;
                            if (strtolower($key) === 'category' && $product['category']) continue;
                        ?>
                            <tr>
                ...
                <?php else: ?>
                <p style="color:var(--ink-3);font-size:.9rem;padding:.5rem 0">No specifications available for this product.</p>
                <?php endif; ?>
            </div>
...
<!-- ── Quote Modal ── -->
<div class="quote-overlay" id="quoteOverlay" onclick="handleOverlayClick(event)">
    <div class="quote-modal">

        <!-- Header -->
        <div class="qm-header">
            <div class="qm-header-icon"><i class="fas fa-file-invoice"></i></div>
            <div>
                <h3>Request a Quotation</h3>
                <p class="qsub">We'll email you a formal quotation for this item.</p>
            </div>
            <button class="quote-close" onclick="closeQuoteModal()" aria-label="Close"><i class="fas fa-xmark"></i></button>
        </div>

        <!-- Hidden product data -->
        <input type="hidden" id="qProductId"    value="<?= $product['id'] ?>">
        <input type="hidden" id="qProductName"  value="<?= htmlspecialchars($product['name'],  ENT_QUOTES) ?>">
        <input type="hidden" id="qProductBrand" value="<?= htmlspecialchars($product['brand'], ENT_QUOTES) ?>">
        <input type="hidden" id="qProductPrice" value="<?= $product['price'] ?>">

        <!-- Product preview strip -->
        <div class="qm-product-strip">
            <div class="qm-prod-img">
                <img src="<?= htmlspecialchars('admin/' . ltrim($product['image'], '/')) ?>"
                     alt="<?= htmlspecialchars($product['name']) ?>"
                     onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                <div class="qm-prod-fallback" style="display:none"><i class="fas fa-laptop"></i></div>
            </div>
            <div class="qm-prod-info">
                <div class="qm-prod-brand"><?= htmlspecialchars($product['brand']) ?></div>
                <div class="qm-prod-name"><?= htmlspecialchars($product['name']) ?></div>
                <div class="qm-prod-price">LKR <?= number_format($product['price'], 2) ?></div>
            </div>
        </div>

        <!-- Form — field names match send_quotation_email.php -->
        <div id="qFormStep">
            <div class="q-row2">
                <div class="q-field">
                    <label for="qFullName"><i class="fas fa-user"></i> Full Name <span class="qreq">*</span></label>
                    <input type="text" id="qFullName" placeholder="John Silva" autocomplete="name">
                    <div class="q-err" id="qFullNameErr"></div>
                </div>
                <div class="q-field">
                    <label for="qPhone"><i class="fas fa-phone"></i> Phone <span class="qreq">*</span></label>
                    <input type="tel" id="qPhone" placeholder="+94 7X XXX XXXX" autocomplete="tel">
                    <div class="q-err" id="qPhoneErr"></div>
                </div>
            </div>
            <div class="q-field">
                <label for="qEmail"><i class="fas fa-envelope"></i> Email Address <span class="qreq">*</span></label>
                <input type="email" id="qEmail" placeholder="you@example.com" autocomplete="email">
                <div class="q-err" id="qEmailErr"></div>
            </div>
            <!-- Validity note -->
            <div class="q-validity-note">
                <i class="fas fa-clock"></i>
                Quotation will be valid for <strong>30 days</strong> from the issue date.
            </div>

            <div class="q-actions">
                <button class="btn-q-cancel" onclick="closeQuoteModal()">Cancel</button>
                <button class="btn-q-submit" id="qSubmitBtn" onclick="submitQuote()">
        ...
        <!-- Success state -->
        <div id="qSuccessStep" style="display:none;" class="qm-success">
            <div class="qm-success-icon"><i class="fas fa-envelope-open-text"></i></div>
            <h4>Quotation Sent!</h4>
            <p>Your quotation <strong id="qSuccessRef"></strong> has been emailed to <strong id="qSuccessEmail"></strong>.<br>It's valid for 30 days — contact us to confirm your order.</p>
            <button class="btn-q-submit" style="width:100%;" onclick="closeQuoteModal()">
...
function openQuoteModal() {
    // Reset to form state
    document.getElementById('qFormStep').style.display  = '';
    document.getElementById('qSuccessStep').style.display = 'none';
    // Clear fields
    ['qFullName','qPhone','qEmail'].forEach(id => {
        const el = document.getElementById(id);
        if (el) { el.value = ''; el.classList.remove('q-invalid'); }
    });
    ['qFullNameErr','qPhoneErr','qEmailErr'].forEach(id => {
        const el = document.getElementById(id); if (el) el.textContent = '';
    });

    document.getElementById('quoteOverlay').classList.add('open');
    document.body.style.overflow = 'hidden';
    setTimeout(() => document.getElementById('qFullName').focus(), 250);
}
...
function handleOverlayClick(e) {
    if (e.target === document.getElementById('quoteOverlay')) closeQuoteModal();
}
document.addEventListener('keydown', e => { if (e.key === 'Escape') closeQuoteModal(); });

/* Inline validation helper */
function qValidate(inputId, errId, required, type) {
    const el  = document.getElementById(inputId);
    const err = document.getElementById(errId);
    const val = el.value.trim();
    el.classList.remove('q-invalid');
    if (err) err.textContent = '';
    if (required && !val) {
        el.classList.add('q-invalid');
    ...
    if (type === 'phone' && val && !/^[\d\s\+\-\(\)]{7,20}$/.test(val)) {
        el.classList.add('q-invalid');
...
function submitQuote() {
    // Validate
    const v1 = qValidate('qFullName', 'qFullNameErr', true,  null);
    const v2 = qValidate('qPhone',    'qPhoneErr',    true,  'phone');
    const v3 = qValidate('qEmail',    'qEmailErr',    true,  'email');
    if (!v1 || !v2 || !v3) return;

    const full_name        = document.getElementById('qFullName').value.trim();
    const phone            = document.getElementById('qPhone').value.trim();
    const email            = document.getElementById('qEmail').value.trim();
    const product_id       = document.getElementById('qProductId').value;
    const product_name     = document.getElementById('qProductName').value;
    const product_brand    = document.getElementById('qProductBrand').value;
    const unit_price       = parseFloat(document.getElementById('qProductPrice').value) || 0;
    const quotation_number = 'QUO-' + Date.now().toString(36).toUpperCase();

    // Build payload that matches send_quotation_email.php expectations
    const payload = {
        full_name,
        phone,
        email,
    ...
    .then(data => {
        if (data.success) {
            // Show success state
            document.getElementById('qFormStep').style.display    = 'none';
            document.getElementById('qSuccessStep').style.display = '';
            document.getElementById('qSuccessRef').textContent    = '#' + quotation_number;
            document.getElementById('qSuccessEmail').textContent  = email;
            showToast('Quotation sent successfully!', 's');
        } else {
            showToast(data.message || 'Failed to send quotation. Please try again.', 'e');
            btn.innerHTML = orig;
            btn.disabled  = false;
        }
    })
